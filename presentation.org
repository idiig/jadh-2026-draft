...
* 这个工作做了什么？
尝试用结构化数据描写kugire并给出多种来源，
讨论
1. *非可视* 的解释要素在结构化数据中的标注
2. 以及标注如何留存信息的来源与信度
...
对于古今和歌的研究，信息交换和传承的空间，注释和传授的说明很多样。
说明也具有针对性。
基于注释和辞典的研究，以及跨领域的知识，20世纪的注释书中为了说明和歌的内容添加了现代语言的翻译。

20世纪的研究者的翻译很多基于注释和享受史的研究，我们也可以看作专业信息传播和阅读史的一个组成部分。

从语言学和知识交流的角度出发，我们目前为止关注了现代日语翻译中体现出来的和歌的解释的多个侧面。

1. 词汇关联空间的差异：一个词再现代日语里必须被补充什么信息
   辞典中选择新脱落，但实践中不可缺的内容。共同和多样性。
   
2. 谓词（bare verb）变换规则：语言理论趋于一致，翻译实践趋于多样。

目前我们的研究从翻译中我们可以得到“理论上应该”的解读和“实践上至少”的差异，
以及实践解读的微观层面的变异。
并开发了很多方法。

但是这些研究目前没有讨论语篇层次上的解读多样性？kugire是一个很好的切口
